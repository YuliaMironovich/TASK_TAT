...
namespace DEV_13
{
    ...
    public class MinNumberOfEmployee: IStrategy
    {
        ...
        //Searsh for the minimum count of employee higher than Junior with fix productivity from the teams available for purchase
        private int SearchMinCountOfNotJunior(int minEmployee, List<List<int>> countOfEmployee, InitialCondition initialCondition, List<int> productivity, List<int> cost)
        {
            for (int i = 0; i < countOfEmployee[0].Count; i++)
            {
                if (productivity[i] == initialCondition.productivity && cost[i] <= initialCondition.cash)
                {
                    if (minEmployee > (countOfEmployee[1][i] + countOfEmployee[2][i] + countOfEmployee[3][i]))
                    {
                        minEmployee = countOfEmployee[1][i] + countOfEmployee[2][i] + countOfEmployee[3][i];
                    }
                }
            }
            return minEmployee;
        ...
        private List<int> FindPossibleTeams(List<int> team, List<List<int>> countOfEmployee, InitialCondition initialCondition, List<int> productivity, List<int> cost, int minEmployee)
        {
            for (int i = 0; i < countOfEmployee[0].Count; i++)
            {
                if (minEmployee == (countOfEmployee[1][i] + countOfEmployee[2][i] + countOfEmployee[3][i]) && productivity[i] == initialCondition.productivity && cost[i] <= initialCondition.cash)
                {
                    team.Add(i);
                }
            }
